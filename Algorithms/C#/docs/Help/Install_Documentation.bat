...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1 /catalogName "VisualStudio12" /locale en-us /wait 0 /operation uninstall /vendor "Vendor Name" /productName "A Sandcastle Documented Class Library" /bookList "A Sandcastle Documented Class Library" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1 /catalogName "VisualStudio12" /locale en-us /wait 0 /operation install /sourceUri "%CD%\Documentation.msha"
